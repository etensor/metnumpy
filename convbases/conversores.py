...
class Conversor():
    ...
    # esta función no la usamos, manual mas garra.
    ...
    # funcion para poder extraer bien el shift del exponente que define el entero
    def bin_ieee_dec_shift(self, num, mod=32):
    ...
    def ieee3264_2n(self, num, shift=0):
        ...
        # if '.' != num[0]
        desnorm_mnt = '1' + \
            str(num[expb+1:expb+1+shift] + '.' + num[expb+1+shift:])
        # expb+2+shift] + '.' + num[expb+2+shift:])
        significando = '1.'+str(num[expb+1:])
